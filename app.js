...
const state = { selectedPrice: 2.19, channelAllocation: { "DTC Online": 40, "Retail/Grocery": 35, "Gym & Office": 25 } };
...
async function load() {
  try {
    ...
    $("#seasonality").innerHTML = rows.map((row) => '<div class="' + (row.seasonalityIndex === peak ? "peak" : "") + '" title="Month ' + row.month + ': ' + row.seasonalityIndex + '"><i style="height:' + row.seasonalityIndex / 1.5 + 'px"></i><span>' + row.month + "</span></div>").join("");
    ...
    $("#recommendation-metrics").innerHTML = '<div><span>Acceptance</span><strong>' + recommendation.keyMetrics.estimatedAcceptancePct.toFixed(1) + '%</strong></div><div><span>Contribution / unit</span><strong>' + euro(recommendation.keyMetrics.contributionMarginPerUnitEur) + '</strong></div><div><span>CAC payback</span><strong>' + (recommendation.keyMetrics.approximateCacPaybackMonths == null ? "N/A" : recommendation.keyMetrics.approximateCacPaybackMonths.toFixed(1) + " mo") + '</strong></div>';
    ...
    $("#calculation-note").textContent = "Mix math: 60% DTC × €1.16 + 20% Retail × €0.63 + 20% Gym × €1.13 = " + euro(recommendedContribution) + " contribution per unit. Payback: €44 CAC ÷ (" + euro(recommendedContribution) + " × 6.144 survey-reported monthly purchases) = " + payback.toFixed(1) + " months. Acceptance is survey response, not a sales-volume forecast.";
    ...
    $("#scenario-note").textContent = "Directional payback range — downside " + downside.toFixed(1) + " mo · base " + payback.toFixed(1) + " mo · upside " + upside.toFixed(1) + " mo. The 3:1 LTV:CAC hurdle requires at least €132 lifetime value at €44 CAC; German LTV is not yet sales-validated.";
